...
export async function generateImage(prompt: string, config: { aspectRatio?: string; imageSize?: string } = {}) {
  // gemini-3.1-flash-image-preview requires a paid API key selected by the user
...
export async function generateVideo(prompt: string, aspectRatio: "16:9" | "9:16" = "16:9"): Promise<string> {
  // Veo models require a paid API key selected by the user
  const key = process.env.API_KEY || process.env.GEMINI_API_KEY || "";
  const ai = new GoogleGenAI({ apiKey: key });
  ...
  if (!downloadLink) throw new Error("Video generation failed or was cancelled.");
